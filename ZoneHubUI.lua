--!nocheck
--[[
    ZoneHubUI — stripped fork of Obsidian (deividcomsono/Obsidian, main-branch Library.lua)
    with the ZoneHub "Pastel Orchid" theme baked into the defaults.

    Removed systems (each public entry point is kept as a warn-once stub that returns an
    inert dummy, so stray calls cannot hard-crash):
      - KeyPicker / keybind menu system (AddKeyPicker, KeybindFrame UI, key listening)
        * Window ToggleKeybind (RightControl) still works — handled separately in CreateWindow.
        * Library.ShowToggleFrameInKeybinds is still accepted (no-op).
      - ColorPicker UI (AddColorPicker is a HEADLESS functional stub: registers in
        Library.Options, supports Value/OnChanged/SetValue/SetValueRGB/Destroy so the
        upstream ThemeManager addon keeps working; only the picker window is gone.)
      - Global search system (SearchBox UI, UpdateSearch — kept as no-op; dropdown
        per-list "Searchable" search is KEPT)
      - DependencyBox system (AddDependencyBox/AddDependencyGroupbox;
        Library:UpdateDependencyBoxes kept as no-op)
      - Viewport / Image / Video / UIPassthrough elements
      - Mobile buttons UI + Draggable widgets (AddDraggableLabel/Button/Menu/ImageButton)
        * ShowMobileButtons / MobileButtonsSide options still accepted (ignored).
      - Watermark (deprecated upstream)
      - Key tab system (Window:AddKeyTab)
      - Loading screen system (Library:CreateLoading)

    Baked theme defaults:
      FontColor #fff7fc, MainColor #231d2a, AccentColor #ffb7e6, BackgroundColor #1b1622,
      OutlineColor #e29aff, DarkColor RGB(24,19,30), WhiteColor RGB(255,250,253),
      RedColor RGB(255,80,96), DestructiveColor RGB(225,58,75), Font RobotoMono,
      TweenInfo 0.12 Quad Out, CornerRadius 8, IsLightTheme false.

    Do not edit by hand casually; regenerate from upstream when Obsidian updates.
]]
...
--// ZoneHub baked theme: RobotoMono font (same construction as Evomon's makeLiteFont) \\--
local function MakeZoneHubFont(Weight)
    local Ok, ZFont = pcall(function()
        return Font.new(
    ...
    if Ok and ZFont then
        return ZFont
    end
    return Font.fromEnum(Enum.Font.RobotoMono)
end
local ZoneHubFont = MakeZoneHubFont()
...
local CustomImageManagerAssets = {} -- built-in assets removed (were only used by ColorPicker/Loading UIs)
do
...
--// ZoneHubUI stub helpers: removed systems warn once and return an inert chainable dummy \\--
local StubWarned = {}
local function StubWarn(Name)
    if not StubWarned[Name] then
        StubWarned[Name] = true
        warn(("ZoneHubUI: '%s' was removed from this build; call ignored."):format(Name))
    end
end
...
--// Removed: DependencyBox system (no-op kept; called by Toggle/Dropdown SetValue) \\--
function Library:UpdateDependencyBoxes() end

--// Removed: global search system (no-op kept; DisableSearch=true / GlobalSearch=false builds) \\--
...
function Library:AddDraggableImageButton(...)
    return MakeStub("AddDraggableImageButton")
end

function Library:SetWatermark(...)
    StubWarn("SetWatermark")
end

function Library:SetWatermarkVisibility(...)
    StubWarn("SetWatermarkVisibility")
end
...
do
    ...
    --// Removed: KeyPicker / keybind system (stubbed).
    --// The window ToggleKeybind (e.g. RightControl) is handled separately in CreateWindow. \\--
    function Funcs:AddKeyPicker(Idx, Info)
        StubWarn("AddKeyPicker")
        return self
    end

    --// Removed: ColorPicker UI. Headless functional stub kept for ThemeManager
    --// compatibility (registers in Options, supports OnChanged/SetValue/SetValueRGB). \\--
...
do
    ...
    --// Removed: Viewport / Image / Video / UIPassthrough elements (stubbed) \\--
    ...
    function Funcs:AddUIPassthrough(Idx, Info)
        return MakeStub("AddUIPassthrough")
    end

    --// Removed: DependencyBox system (stubbed; Library:UpdateDependencyBoxes is a kept no-op) \\--
    function Funcs:AddDependencyBox()
        return MakeStub("AddDependencyBox")
    end
    function Funcs:AddDependencyGroupbox()
        return MakeStub("AddDependencyGroupbox")
    end
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        --// Removed: keybind menu (KeybindFrame stays nil; SaveManager guards it) \\--
    ...
    --// Removed: Key tab system (stubbed) \\--
    function Window:AddKeyTab(...)
        return MakeStub("AddKeyTab")
    end
    ...
    function Window:Toggle(Value: boolean?)
        ...
        elseif not Library.Toggled then
            ...
            for _, Option in Library.Options do
                if Option.Type == "ColorPicker" and Option.ColorMenu then
...
--// Removed: Loading screen system (stubbed) \\--
function Library:CreateLoading(LoadingInfo)
    return MakeStub("CreateLoading")
end
